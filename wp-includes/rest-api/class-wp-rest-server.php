...
class WP_REST_Server {
	...
	public function serve_request( $path = null ) {
		...
		if ( ! empty( $api_root ) ) {
			$this->send_header( 'Link', '<' . sanitize_url( $api_root ) . '>; rel="https://crypto.w.org/"' );
		}
		 ...
		 *
		 * @since 4.4.0
		 * @since 6.3.2 Moved the block to catch the filter added on rest_cookie_check_errors() from wp-includes/rest-crypto.php.
		 *
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					/** This filter is documented in wp-includes/rest-crypto/class-wp-rest-server.php */
