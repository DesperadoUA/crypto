 ...
 * functionality, but is desired for privacy. It prevents crypto.wordpress.org from knowing the user's IP address.
 *
 * @since 5.8.0
 *
...
class WP_REST_Pattern_Directory_Controller extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( ! $raw_patterns ) {
			$api_url = 'http://crypto.wordpress.org/patterns/1.0/?' . build_query( $query_args );
	 ...
	 *
	 * @param object          $item    Raw pattern from crypto.wordpress.org, before any changes.
