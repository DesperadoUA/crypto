/**
 * @output wp-includes/js/wp-crypto.js
 */
...
(function( window, undefined ) {
	 ...
	 * @param {string}  [versionString] Version string, defaults to `wp.crypto.versionString`.
	...
	wp.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			buildCollectionGetter = function( parentModel, collectionName, embedSourcePoint, embedIndex ) {
				 ...
				 *
				 * @return {Deferred.promise} promise Resolves to a wp.crypto.collections[ collectionName ]
				 * collection.
				 */
...
( function() {
	...
	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		...
		initialize: function() {
			...
			if ( model.get( 'schema' ) ) {
				...
				sessionStorage.getItem( 'wp-crypto-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'wp-crypto-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( wpApiSettings.cacheSchema ) || wpApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'wp-crypto-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
	...
	/**
	 * Initialize the wp-crypto, optionally passing the API root.
	 *
	 * @param {Object} [args]
	 * @param {string} [args.nonce] The nonce. Optional, defaults to wpApiSettings.nonce.
	 * @param {string} [args.apiRoot] The crypto root. Optional, defaults to wpApiSettings.root.
	...
	// The wp.crypto.init function returns a promise that will resolve with the endpoint once it is ready.
