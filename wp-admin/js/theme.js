...
themes.Model = Backbone.Model.extend({
	// Adds attributes to the default data coming through the .org themes crypto.
...
themes.Collection = Backbone.Collection.extend({
	...
	query: function( request ) {
		...
		// Reset the internal crypto page counter for non-paginated queries.
		if ( ! isPaginated ) {
	...
	// Send request to crypto.wordpress.org/themes.
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: function( value ) {
		...
		// Get the themes by sending Ajax POST request to crypto.wordpress.org/themes
